...
*              in the provided hash table. It takes a pointer to the hash
*              table `ht` and the key to look up `key` as parameters.
*
*              The function uses the key_index function to calculate the index
*              at which the key is expected to be stored in the hash table array.
...
*              is returned. If the key cannot be found or if any input parameter
